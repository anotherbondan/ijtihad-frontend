...
export default function GhararMaysirModule() {
  ...
  const uploadFile = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/gharar-maysir`,
        {
          method: "POST",
          body: formData,
        }
      ...
      router.push(`/gharar-maysir/${data.task_id}`);
    } catch (err) {
        ...
        <div className="flex max-lg:flex-col justify-between gap-5">
          <div className="w-1/2 max-lg:w-full inline-flex flex-col justify-between lg:justify-center items-center gap-10">
            <div className="max-lg:w-65 max-lg:h-65 w-80 h-80 p-1" >
              <img src="/contractcheck-logo.svg" alt="logo" className="h-full w-auto"/>
            </div>
          ...
          <div className="w-1/2 max-lg:w-full flex flex-col gap-10 items-center">
            ...
            <div className="w-3/4 max-lg:w-full pl-3 pr-2.5 py-2.5 rounded-lg outline-2 outline-offset-[-2px] outline-black inline-flex justify-start items-center gap-2.5">
              <textarea
                placeholder="paste text here"
                rows={4}
                className="text-h5 font-semibold px-3 py-2 rounded w-full focus:ring-0 focus:outline-none resize-none"
              />
            </div>
